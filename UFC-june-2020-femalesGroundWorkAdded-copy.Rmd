...
After extracting the features to run Machine Learning (ML) on, R's caret package is used to test out various algorithms for accuracy in hits landed based on extracted features using random forest, glm, rpart, knn, and gbm. Then those same selected features of that data set are used to predict the same outcome in Python's sklearn and keras package for ML using the random forest and gradient boosted classifiers as well as the multinomial naive bayes. A set up in python is also arranged for deep neural nets training and testing. But the random forest and gradient boosted classifiers of Python scored 100% accuracy before trying the DNNs. The reticulate and python environment were having issues with using tensorflow's added keras, so it was not implemented.
...
[Machine Learning with R packages](#machine-learning-with-r-packages)
[Machine Learning with Python Packages sklearn and Keras](#machine-learning-with-python)
...
```{python, eval=FALSE}
y_train_pred6 = model6.predict_classes(Felicia_hits_train, verbose=0)
print('First 3 predictions: ', y_train_pred6[:3])
```


```{python, eval=FALSE}
y_train_pred6 = model6.predict_classes(Felicia_hit_train, 
                                     verbose=0)
```
...
```{python, eval=FALSE}
y_test_pred6 = model6.predict_classes(Felicia_hits_test, 
                                    verbose=0)
```
